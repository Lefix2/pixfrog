...
#include "encoder_nrz.h"  // source_pixel / corrected / transformed_rgb (shared transform)
...
size_t emit_clocked_bit(uint16_t* out, uint16_t data_mask, uint16_t clk_mask, bool bit_value,
                        uint16_t spc) {
...
size_t emit_clocked_byte(uint16_t* out, uint16_t data_mask, uint16_t clk_mask, uint8_t byte,
                         uint16_t spc) {
    size_t s = 0;
