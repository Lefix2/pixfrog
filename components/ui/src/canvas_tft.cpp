...
int canvas_width() { return tft_width(); }
int canvas_height() { return tft_height(); }

void canvas_clear(Color bg) {
    for (int i = 0; i < tft_width(); i++) s_row_buf[i] = bg.v;
...
void canvas_fill_rect(int x, int y, int w, int h, Color c) {
    ...
    for (int i = 0; i < clamped_w; i++) s_row_buf[i] = c.v;
    for (int row = y; row < y + h; row++)
        tft_draw_bitmap(x, row, x + clamped_w, row + 1, s_row_buf);
}

void canvas_draw_text(int x, int y, const char* str, Color fg, Color bg,
                      uint8_t scale) {
    if (!str || !*str) return;
    ...
    for (int i = 0; i < tw * th; i++) s_text_buf[i] = bg.v;
