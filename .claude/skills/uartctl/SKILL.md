...
Commands: `version status stats chstat` · `global [<key> <val>]` · `ch <n> [<key> <val>]` · `dmxw <uni> <slot> <hex>` · `dmxr <uni> [start len]` · `pixr <ch> [start len]` · `cal [-1..3]` (3 = GPIO bit-bang probe of the 16 bus pins, reboot to restore) · `loglevel <none..verbose>` · `factory-reset` · `reboot`. Key lists + ranges: AGENT.md "Control console (UART)".
